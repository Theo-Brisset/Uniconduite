...
window.onload = function() {
    ...
    for (let i = 0; i < typeRoute.length; i++) {
        ...
        option.id = `typeRoute${typeRoute[i].typeRouteId}`;
        
        label.htmlFor = option.id;
        label.textContent = typeRoute[i].typeRouteNom;

        selectRoute.appendChild(option);
        selectRoute.appendChild(label);
    }
    //trafic
    ...
    for (let i = 0; i < manoeuvres.length; i++) {
        ...
        let label = document.createElement("label");
        option.type = "checkbox";
        option.name = "manoeuvre";
        option.value = manoeuvres[i].manoeuvreId;
        option.id = `manoeuvres${manoeuvres[i].manoeuvreId}`;
        
        label.htmlFor = option.id;
        label.textContent = manoeuvres[i].manoeuvreNom;

        selectManoeuvre.appendChild(option);
        selectManoeuvre.appendChild(label);
    }
